...
def test_dyfi():
    ...
    np.testing.assert_equal(dataframe['NRESP'].sum(), 16202)

    # Test that stddev and resp carry throuh
    try:
        minversion = '0.8.17'
        assert impactutils.__version__ >= minversion
    except AssertionError:
        print('impactutils version', minversion, 'required to process intensity parameters.')
        print('If you do not work with intensity data, you can safely ignore this warning.')
        return
        
    np.testing.assert_almost_equal(dataframe['INTENSITY_STDDEV'].sum(), 39.548)
